 ...
 */
router.post('/', async (req: Request<unknown, unknown, CreateMovieBody>, res: Response) => {
  try {
    const {
      tmdb_id,
      title,
    ...
    .then((response:any)=>{
        const movie = response.rows as Movie
        res.status(201).json(movie);
    });

  } catch (error) {
    // Hantera dubblett (UNIQUE constraint-överträdelser)
    if (error instanceof Error && error.message.includes('duplicate key value violates unique constraint "movies_tmdb_id_key"')) {
 ...
 */
router.put('/:id', async (req: Request<{ id: string }, unknown, UpdateMovieBody>, res: Response) => {
  try {
    const { id } = req.params;
    const {
      status,
    ...
    if (personal_rating !== undefined) {
      updates.push(`personal_rating = '${personal_rating}'`);
    ...
    if (date_watched !== undefined) {
      updates.push(`date_watched = '${date_watched}'`);
